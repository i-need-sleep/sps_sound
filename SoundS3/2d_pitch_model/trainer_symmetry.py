    ...
    def train(self):
        ...
        for i in range(iter_num, self.max_iter_num):
            ...
            T, Tr = make_translation_batch(
                batch_size=BATCH_SIZE * DT_BATCH_MULTIPLE, 
                dim=np.array([1, 1]),
                t_range=self.t_range, 
            )
            R, Rr, theta = make_random_rotation_batch(
                batch_size=BATCH_SIZE * self.r_batch_multiple,
            ...
            R_loss = self.batch_symm_loss(
                ...
                data[:, 1:, :, :, :], z_gt_p, z0_rnn_extended, T_sample_points, DT_BATCH_MULTIPLE,
                lambda z: symm_trans(z, R), lambda z: symm_trans(z, Rr), z_gt_cr_extended[:, :-1, :]
            )
